...
const CreateRequest = ({campaign,id}) => {
  ...
  const handleCreate = async (e)=>{
    ...
    try{
      ...
      await campaign.methods.createRequest(name,web3.utils.toWei(eth,'ether'), accounts[0]).send({
        from:accounts[0]
      })
  ...
  return (
    <div>
     ...
     {open && <div style={{backgroundColor: 'rgba(30,34,42, 0.7)'}} onClick={handleClick} className='w-[100vw] md:w-[98.8vw] z-[100] flex items-center justify-center h-[100vh] top-[0px] left-[0] absolute'>
          ...
          <h3 className="text-lg font-bold">Create a Request</h3>
          <div className="form-control mt-4 mb-3">
            ...
            <label className="input-group">
              ...
              <input value={name} onChange={(e)=>setName(e.target.value)} type="text" placeholder="Request" className="input w-[100%] input-bordered" />
            </label>
          </div>
          <div className="form-control">
            <label className="label">
              <span className="label-text">Amount you want to withdraw</span>
            </label>
            <label className="input-group">
          ...
          {!loading && <button onClick={handleCreate} className="btn btn-primary btn-wide mt-6">Add Request</button>}
